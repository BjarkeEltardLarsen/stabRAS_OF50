...
{
  this->nut_ = Cmu_*sqr(k_)/max(epsilon_,1.2603*lambda2_*C2_/C1_*2.0*magSqr(symm(fvc::grad(this->U_)))/(2.0*magSqr(skew(fvc::grad(this->U_)))+pOmegaSmall_)*epsilon_); //nut as suggested by Larsen and Fuhrman (2018);
