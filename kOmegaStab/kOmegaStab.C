...
{
  this->nut_ = k_/max(omega_,lambda2_*beta_/(Cmu_*gamma_)*2.0*magSqr(symm(fvc::grad(this->U_)))/(2.0*magSqr(skew(fvc::grad(this->U_)))+pOmegaSmall_)*omega_); //nut as suggested by Larsen and (Fuhrman 2018)
